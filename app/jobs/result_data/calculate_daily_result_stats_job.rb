...
module ResultData
  class CalculateDailyResultStatsJob < ApplicationJob
    ...
    def perform
      calculated_values =
        ResultsData.where(timestamp: Date.today.beginning_of_day..Date.today.end_of_day)
          .group(:subject)
      ...
      calculated_values.map{ |calculated_value| DailyResultStat.create(calculated_value.attributes) }
    end
  end
end
