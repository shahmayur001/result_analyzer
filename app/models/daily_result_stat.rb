...
class DailyResultStat < ApplicationRecord
  ...
  def self.calculate_average_for_subject_with_limit(subject, fetch_count)
    ...
    where(id: recent_data_ids).select("SUM(daily_volume) AS monthly_result_count_used, AVG(daily_low) AS monthly_avg_low, AVG(daily_high) AS monthly_avg_high").first
  end
end
